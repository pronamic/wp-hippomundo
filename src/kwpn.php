...
<style scoped>
    ...
    .hm-lrt .flag {width: 16px;height: 11px;background:url(<?php echo $results->base_url;?>img/flags/flags.png) no-repeat;}
...
<div class="hm-lrt">
    <div class="hm-powered">Powered by <a href="http://www.hippomundo.com/nl" target="_blank">Hippomundo</a> &copy;</div>
...
{
    public $base_url = 'http://www.hippomundo.com/';
    public $rider_url;
    public $pedigree_url;
    public $studbook;
    public $api_key;
    ...
    protected function formatFlag($result, $base_url)
    {
        ...
        if (!empty($result['iso_code'])) {
            $return .= '<img src="' . $base_url . 'img/blank.gif" class="flag flag-' . strtolower($result['iso_code']) . '" title="' . $result['country'] . '">';
        }
        return $return;
    }
    ...
    protected function formatBreeder($result, $base_url)
    {
        ...
        if (strlen($result['breeder']) > 1) {
            ...
            if ($result['breeder_published'] !== '0' && strlen($result['breeder_website']) > 1) {
                ...
                $breeder .= $result['breeder'] . '&nbsp;<a target="_blank" href="' . $base_url . '/nl/fokker-link">';
                $breeder .= '<img src="' . $base_url . 'addons/shared_addons/themes/hippomundo/img/info-icon-grey.png" alt="Advertise your link here"></a>';
            }
            $breeder .= '</span>';
        }
        return $breeder;
